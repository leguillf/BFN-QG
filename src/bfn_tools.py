...
import tools_switchvar as switchvar
from tools_math import gaspari_cohn, hat_function, L2_scalar_prod
...
class bfn_qg1l(object):
    ...
    def update_parameter(self, model_state, Nold, N, Wbc, way=1):
        ...
        model_state_updated = model_state.copy()

        psi =  self.g/self.f * model_state[0] # 1st variable ssh
        K = model_state[2].copy() # 3rd variable (K)
        ...
        model_state_updated[2] = K

        Ntot = self.n_mod_var*self.ny*self.nx
        model_state_updated = model_state_updated.reshape(1, Ntot)

        return model_state_updated
                ...
                if os.path.isfile(file_obs_save):
                    ...
                    if flag_plot > 1:
                        ...
                        plt.suptitle('Nudging on ' + varname + ' at ' +
                                     ...
                                     ' & sigma = ' + str(key[0]) + ')')
                        im1 = ax1.pcolormesh(lon2d, lat2d, obs_projected)
...
' observations, grid has to be the same as the model one')
    ...
    if flag_plot is not None and flag_plot > 1:
        ...
        if len(lonobs.shape) == 2:
            im0 = ax0.pcolormesh(lonobs, latobs, varobs)
        else:
        ...
        im1 = ax1.pcolormesh(lon2d, lat2d, proj_var)
